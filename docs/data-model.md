...
| `sequence_type` | string | NULL は `'(unknown)'` に塗り潰される |
| `organism_normalized` | string | NULL は `'(unknown)'` に塗り潰される |
| `in_chip_atlas` | bool | not null |
...
| `organism_normalized` | string | NULL は `'(unknown)'` |
| `in_chip_atlas` | bool | |
| `sample_count` | int64 | distinct accession 数 |
