...
| `source_system` | string | not null | `chip-atlas-hg38` / `chip-atlas-mm10` / `rnaseq-human` (= bsllmner-viewer の系統 ID)。ChIP-Atlas との接続点はこの列で表現する (旧 `in_chip_atlas` を `source_system LIKE 'chip-atlas-%'` に置換、`chip_atlas_genome` は `lib/chip_atlas.py:SOURCE_SYSTEM_TO_GENOME` の 3 行 dict で導出) |
