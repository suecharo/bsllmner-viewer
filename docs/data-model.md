...
PoC スコープでは BioSample 単位の連携 (`in_chip_atlas` + ChIP-Atlas top jump) で十分なので、SRX 単位の `chip_atlas_links.parquet` は作らない (本ドキュメント末尾の節を参照)。
...
## chip_atlas_links.parquet (PoC スコープ外)

design-memo §6 では作る計画だったが、PoC スコープでは作らない。理由を整理:

- bsllmner-mk2 の input JSONL には **BioSample accession までしか入っていない** (`Ids.Id[*].namespace = "SRA"` で来るのは `DRS` 等の Sample ID、SRX = Experiment ID は無い)
- ChIP-Atlas の deep link / BigWig / peak BED URL は SRX 単位で組み立てる必要がある:
  - BigWig: `https://chip-atlas.dbcls.jp/data/<genome>/eachData/bw/<SRX>.bw`
  - Peak BED: `https://chip-atlas.dbcls.jp/data/<genome>/eachData/bed05/<SRX>.05.bed`
- accession → SRX のマッピングは ChIP-Atlas の `experimentList.tab` (https://chip-atlas.dbcls.jp/data/metadata/experimentList.tab) を解析する必要がある
- ChIP-Atlas の検索 UI (https://chip-atlas.org/peak_browser, https://chip-atlas.org/) は accession を URL クエリで受け付けない
- PoC では「BioSample 単位の gap 発見 / cohort 持ち出し」が主軸であり、SRX 単位の track 表示 (S4 / S5a) は scope 外 (`docs/ui.md` 参照)
- → 本 PoC では `in_chip_atlas` flag だけで「ChIP-Atlas top jump + accession 表示」して、ユーザーに手で検索してもらう

将来 IGV.js (S5a) や Peak Browser deep link (S4) を導入する場合は、本節を復活させて `experimentList.tab` から (`srx`, `accession`, `genome`, `antigen`, `cell_type_chipatlas`) を抽出する parquet を別途生成する。URL 列は機械的に組み立て可能なので保存しない。
