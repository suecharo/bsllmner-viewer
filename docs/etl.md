...
bsllmner-mk2 の select 結果 (`select_*.json` + `bs_entries_*.jsonl`) と、viewer 自身が用意した ontology OWL から、bsllmner-viewer の parquet ファイル群を生成する pipeline の仕様。
...
4 系統。系統 = (source_system, organism, default_sequence_type) の組。ChIP-Atlas との接続点は `source_system` の prefix で派生する (`source_system LIKE 'chip-atlas-%'`) ので独立した bool 列は持たない。genome (`hg38` / `mm10`) は `lib/chip_atlas.py:SOURCE_SYSTEM_TO_GENOME` dict で導出:

| source_system | organism | default_sequence_type | input | result |
|---|---|---|---|---|
| `chip-atlas-hg38` | Homo sapiens | (per-SRX, `experimentList.tab` 由来) | `data/chip-atlas-hg38/input/bs_entries_chipatlas_hg38_part*.jsonl` (7 chunks, 各 ~44 MB) | `data/chip-atlas-hg38/result/select_chipatlas_hg38_part*.json` (7 chunks, 各 ~199 MB) |
| `chip-atlas-mm10` | Mus musculus | (per-SRX, `experimentList.tab` 由来) | `data/chip-atlas-mm10/input/bs_entries_chipatlas_mm10_part*.jsonl` (7 chunks, 各 ~50 MB) | `data/chip-atlas-mm10/result/select_chipatlas_mm10_part*.json` (7 chunks, 各 ~170 MB) |
| `rnaseq-human` | Homo sapiens | `RNA-Seq` | `data/rnaseq-human/input/bs_entries_YYYY-MM-01_YYYY-MM-DD.jsonl` (148 files: 5y 2021-04〜2026-03 = 60、past = 87 monthly + 1 merged `2008-01_2013-12`) | `data/rnaseq-human/result/select_rnaseq_human_{5y,past}_*.json` (148 files; 5y 60 + past 88) |
| `rnaseq-mouse` | Mus musculus | `RNA-Seq` | `data/rnaseq-mouse/input/bs_entries_YYYY-MM-01_YYYY-MM-DD.jsonl` (150 files: 1 merged `2008-01_2013-12` + 149 monthly 2014-01〜2026-05; 2026-05 は空入力) | `data/rnaseq-mouse/result/select_rnaseq_mouse_*.json` (149 files; 2026-05 は空入力で SKIP) |

系統定義は `src/bsllmner_viewer/etl/sources.py` に table 駆動で書く（path 解決と (source_system, file) → entries iterator は系統定義から逆引きできる）。

### 2. Ontology OWL (`${BSLLMNER_VIEWER_ONTOLOGY_DIR}/`, default `${BSLLMNER_VIEWER_DATA_DIR}/ontology/`)

各 ontology につき **subset OWL** と **フル OWL** の 2 種類を読む（data-model.md「対象 ontology と source ファイル」参照）:

- subset OWL → `term_id` 集合 + `rdfs:label` の抽出元
- フル OWL → `rdfs:subClassOf` の抽出元（subset OWL は subset 生成時に親参照が剥がされているため）

viewer 側で完結する 2 段階の事前準備で揃える:

1. **upstream OWL の download**: `uv run python scripts/fetch_ontology_owls.py` で `cellosaurus.obo` / `cl.owl` / `efo.owl` / `uberon.owl` / `mondo.owl` / `chebi.owl` を `data/ontology/` に DL する (`scripts/_http_cache.py` 経由で size + ETag/Last-Modified の二段判定 + resume 対応)
2. **subset OWL の生成**: `bash scripts/build_ontology_subsets.sh` を **host** から実行 (内部で `obolibrary/robot:latest` を `docker run` で叩くので Docker-in-Docker を避ける都合)。生成物は `cellosaurus.owl` (`.obo` → ROBOT convert) + `cl_{human,mouse}_subset.owl` + `uberon_{human,mouse}_subset.owl` + `mondo_human_subset.owl` + `chebi_subset.owl`。SPARQL CONSTRUCT テンプレートは [`sh-ikeda/ontology-constructor-for-bsllmner`](https://github.com/sh-ikeda/ontology-constructor-for-bsllmner) を `work/` に clone して使う

事前条件 (host 側): Docker (ROBOT image を引ける) と git。`chebi_subset.owl` の生成は ROBOT に `-Xmx24g` Java heap を渡すので host に 24 GB 以上の RAM が必要。subset 生成は idempotent (既存ファイルがあれば skip、`--force` で強制再生成)。
...
`publication_date` の年抽出: `datetime.fromisoformat()` で parse して `.year`。`+09:00` のような offset 付きでも parse 可能（Python 3.11+ で `Z` も OK、PoC 想定の 3.12 では問題なし）。さらに **ETL 実行年 (`datetime.now(UTC).year`) より大きい年は null に塗り潰す** (NCBI BioSample の embargo 解除予定日が `publication_date` に入って未来年になるケースを除外。詳細は `docs/data-model.md` の `submission_year` 列定義参照)。

## SelectResult の streaming load

`select_*.json` 1 件は ChIP-Atlas chunk で ~200 MB、rnaseq-human の月次で ~50〜250 MB の幅 (合計で chip-atlas 各系統 1.2〜1.4 GB / rnaseq-human 約 15 GB)。`json.load()` 一発はメモリ的に重い。
