...
filter state (`organism_normalized` / `submission_year` range / `source_system` / `sequence_type`) は `st.session_state` 経由で全画面共通管理する。`SampleFilters` は `@dataclass(frozen=True)` で hashable、list field は tuple 化されているため `@st.cache_data` の cache key にそのまま渡せる。filter widget は `_filters.py:sidebar_filters()` 1 箇所で組み立て、page 間 navigation で session_state key (`filter_organism` / `filter_source` / `filter_sequence_type` / `filter_year`) が保持される。

ChIP-Atlas との接続点は **`source_system LIKE 'chip-atlas-%'`** の derived predicate に統一する (旧 `in_chip_atlas` filter / SampleFilters 列は廃止)。Sequence type の絞り込みは `sequence_type` multiselect 1 軸で行う。これにより「ChIP-Atlas 由来か」「assay type 別」の意味的な役割が明確に分離される。
...
- `term_sample_count(field, term_id, filters)` は 1 term ぶんの `COUNT(DISTINCT accession)` と `COUNT(DISTINCT CASE WHEN s.source_system LIKE 'chip-atlas-%' THEN s.accession END)` を返す（heatmap pivot の縮約版、ChIP-Atlas 派生は source_system 経由）
...
1. **Top metrics** (現状維持): Runs / BioSamples / of which ChIP-Atlas (`source_system LIKE 'chip-atlas-%'` の derived) / Facts / Ontology terms の 5 カード
2. **F1 Yearly submission trend** — `submission_year` × `source_system` の積み上げ area (`px.area`、color = `source_system`)。「いつ・どの系統がどれだけ来たか」
3. **F2 Organism / Source / Sequence type split** — 横並びで `organism_normalized` donut + `source_system` donut + `sequence_type` donut の 3 連 (`px.pie(hole=0.5)`)。`'mixed'` / `'(unknown)'` slice は灰色固定 + 末尾配置で de-emphasize して dominant な assay 信号を希釈しないようにする
...
- `summary_counts_fast(con) -> dict[str,int]` (上部 metric。runs/facts/ontology の count は parquet metadata から取って 13.3M scan を回避)
- `samples_by_year_source_fast(con) / samples_by_year_source(con)` (F1)
- `samples_by_organism_fast(con) / samples_by_organism(con)` (F2 左)
- `samples_by_source_fast(con) / samples_by_source(con)` (F2 中央)
- `samples_by_sequence_type_fast(con) / samples_by_sequence_type(con)` (F2 右)
- `field_facts_status_fast(con) / field_facts_status(con)` (F3, F4)
- `top_terms_overall_fast(con, field, top_n) / top_terms_overall(con, field, top_n)` (F5)

`_fast` 系は `agg_*.parquet` を読む fast path、`_fast` 無し系は live `samples` / `facts` を読む fallback。UI 側は **常に `has_dashboard_aggregates(con)` を 1 度判定して fast/live を分岐**する (詳細は本ドキュメント末尾 §「Fast path (agg_*.parquet)」)。
...
- **filter** (sidebar 共通):
  ...
  - `source_system` (multi-select)。ChIP-Atlas に絞りたければ `chip-atlas-hg38` / `chip-atlas-mm10` を選ぶ (旧 `in_chip_atlas` radio は廃止、source_system 1 軸に統一)
  - `sequence_type` (multi-select)。`'(unknown)'` sentinel を末尾に必ず含めて null sample にも触れられるようにする

### 表示

- Plotly heatmap
- 各セルの値: 該当 cohort の `COUNT(DISTINCT accession)`
- overlay: 「うち N 件 overlay 軸該当」を hover text に出す (下記 display mode の `secondary_count`)
- 空白セル (sample 0) は明示的に色分け (gap が一目で分かるように)

### 表示モード (display mode selector)

heatmap 上部に radio で 4 モード切替を置く。`gap_heatmap_pivot` は overlay 軸が選べる `secondary_count` (= `COUNT(DISTINCT CASE WHEN <overlay predicate> THEN accession END)`) を返し、UI 側で color 軸の値を差し替える:

| モード | color 軸の値 | 用途 |
|---|---|---|
| **BioSample count** (default) | `sample_count` | 既存挙動 |
| **Overlay count** | `secondary_count` | overlay 軸 (下記) に絞った heatmap |
| **Overlay ratio** | `secondary_count / sample_count` (sample_count=0 は NaN) | カバー率 (0.0〜1.0) |
| **Gap only (sample > 0, overlay = 0)** | `sample_count` だが、`secondary_count > 0` のセルは NaN にマスク | 「研究はあるが overlay 軸に乗っていない」セルだけを赤く強調 |

color scale はモードごとに変える: count 系は `Blues`、ratio は `RdYlGn` (低 ratio = 赤)、gap only は `Reds`。

### Overlay 軸 (overlay axis selector)

display mode の左隣に selectbox で overlay 軸を選ぶ。`gap_heatmap_pivot(..., overlay_axis=...)` の引数に渡し、`secondary_count` の SQL CASE WHEN predicate が決まる:

| Overlay axis | predicate | 既存の動き |
|---|---|---|
| **ChIP-Atlas** (default) | `s.source_system LIKE 'chip-atlas-%'` | ChIP-Atlas 系統由来 BS の overlap (旧 `chip_atlas_count` 相当) |
| **Sequence type = ChIP-Seq** | `s.sequence_type = 'ChIP-Seq'` | ChIP-Seq 1 種に絞った overlap |
| **Sequence type = ATAC-Seq** | `s.sequence_type = 'ATAC-Seq'` | 同上 |
| **Sequence type = RNA-Seq** | `s.sequence_type = 'RNA-Seq'` | 同上 |
| **Sequence type = (他の `_SEQ_TYPE_ORDER` 値)** | `s.sequence_type = <selected>` | UI で動的に展開 |

overlay 軸を切替えても sidebar の filter は不変。「ChIP-Atlas overlay」と「ChIP-Atlas に絞った heatmap (= filter)」は意味が違う (前者は cell 単位の overlay 比較、後者は母数自体を絞る) ので両者を分けたまま提示する。
...
- 各 term について `facts × samples` を当てて `sample_count` / `secondary_count` を計算
- `px.sunburst` が要求する long-form `(term_id, parent_term_id, label, sample_count, secondary_count, depth)` を返す。root term の `parent_term_id` は空文字 (Plotly の sunburst root 表現)
...
- 既存 `gap_heatmap_pivot` の戻りそのものが long-form `(x_term_id, x_label, y_term_id, y_label, sample_count, secondary_count)` で sankey に必要な形と同じ。新規関数は内部で `gap_heatmap_pivot` を呼んで sample_count > 0 の cell だけに絞り、`x_label` / `y_label` の prefix で重複しないように `{x_field}: {label}` / `{y_field}: {label}` の node 名を組み立てて返す形にする (Plotly Sankey は node 名 unique 要求)
...
- sample table 列 (左から):
  - `accession` (BioSample)
  - `organism_normalized` / `submission_year` / `title` / `source_system` / `sequence_type`
...
- `project` は main sample table には出さない (内容が空 or BioProject の bp4 行で実用情報が薄い)。ChIP-Atlas BigWig / Peak BED は下記 Per-SRX 展開 table 側で `lib/chip_atlas.py:SOURCE_SYSTEM_TO_GENOME` から `source_system` 経由で genome を導出して URL を組み立てる (samples / cohort_srx_links に `chip_atlas_genome` 列を持たせず、純関数 helper 1 箇所に集約)
...
| S4 | ChIP-Atlas BigWig | `source_system LIKE 'chip-atlas-%'` の sample に紐づく全 SRX | `bigwig_url(source_system, srx)` (`lib/chip_atlas.py`) |
| S5 | ChIP-Atlas Peak BED (q < 1E-05) | 同上 | `peak_bed_url(source_system, srx)` (`lib/chip_atlas.py`) |

ChIP-Atlas BigWig / Peak BED は SRX が ChIP-Atlas に登録されているか保証しない (404 になり得る)。`source_system` は系統由来で「ChIP-Atlas 系統の sample である」しか担保しないため、UI は「hint」として提示する。genome 値 (`hg38` / `mm10`) は `SOURCE_SYSTEM_TO_GENOME` dict から derived (rnaseq-human など genome 紐付けの無い系統は None で URL を出さない)。
...
- データソース: `lib/aggregation.py:cohort_srx_links(con, accessions, limit=500)`
  - メイン table から取った BioSample accession list を渡し、`srx_links JOIN samples` を accession で絞り込んで long-form を返す (`source_system` は samples 側からの carry-over)
  - 列: `accession` (BioSample) / `srx` / `bioproject` / `sra_study` / `sra_sample` / `status` / `source_system`
  - 並び順: `accession, srx`
  ...
  - UI 側は `source_system` を `lib/chip_atlas.py:bigwig_url` / `peak_bed_url` に渡して URL を組み立てる (chip_atlas_genome 列は samples / cohort_srx_links に持たない)
...
- `lib/aggregation.py:bubble_dataset(field, filters, top_n, roll_up_depth=None)` で `(submission_year, term_id, label, organism_normalized, sample_count, secondary_count)` の年単位 long-form data を取得。`roll_up_depth` 指定時は heatmap と同じく `_axis_facts_sql` の rolled CTE を経由し、leaf を depth=N の祖先に置換した上で集計
- `cumulative_bubble_dataset(field, filters, top_n, roll_up_depth=None)` は `bubble_dataset` を呼び `(term_id, organism_normalized)` ごとに year で reindex + 0 fill + cumsum。累積 column は `sample_count_cum` / `secondary_count_cum`
...
- **Treemap tab (A11)** は `term_hierarchy_breakdown(field, filters, root_term=None, max_depth, roll_up_depth=None, by_year=True)` を呼ぶ。Sunburst (画面 1 B3) と同じ集計関数を年次対応にした版。`by_year=True` のとき各 year × 各 term の sample_count を返す (`(year, term_id, parent_term_id, label, depth, sample_count, secondary_count)`)、`False` のとき year を抜いた全期間集計を返す
...
6. **D7 Mixed BS の SRX-level 組成** — `samples.sequence_type = 'mixed'` の BS について、`srx_links.parquet` を `(accession, sequence_type)` で集約し、組成パターン (`{'ChIP-Seq': 2, 'ATAC-Seq': 1}` のような multiset) を頻度順 horizontal bar で。`samples` 側で `'mixed'` の 1 ラベルに丸まる情報を SRX-level で復元する curation 用 chart。pattern → count + sample_count の 2 系列 bar、Top 20 まで表示

### 集計クエリ概要

- `mapping_status_matrix_fast(con, filters) / mapping_status_matrix(con, filters)` (D1)
- `mapping_status_over_time_fast(con, filters) / mapping_status_over_time(con, filters)` (D2)
- `top_unmapped_values(con, field, top_n, filters) -> DataFrame[value, n, sample_count]` (D3、`term_id IS NULL AND value IS NOT NULL`)。live のみ (agg では unmapped raw value を残せないため)
...
- `mixed_bs_srx_composition(con, filters, top_n=20) -> DataFrame[pattern, pattern_label, n_bs, n_srx]` (D7、`srx_links` を `(accession, sequence_type)` で集約 → multiset を tuple 化 → 頻度集計)

sidebar filter は D1/D2/D3/D5/D7 に適用する。D4 は run 単位のメタなので filter 非適用。`@st.cache_data` で UI 側 cache。`*_fast` のある関数は `has_dashboard_aggregates(con)` で分岐 (page 上部 1 度判定)。
...
| 関数 | 用途 |
|---|---|
| `get_conn() -> duckdb.DuckDBPyConnection` | `@st.cache_resource` で 1 個共有。`${BSLLMNER_VIEWER_DATA_DIR}/parquet/*.parquet` を `CREATE VIEW` で曝す。connection 作成時に `PRAGMA preserve_insertion_order=false` / `enable_object_cache=true` / `threads` / `memory_limit` / `temp_directory` を env (`BSLLMNER_VIEWER_DUCKDB_*`) で設定する |
| `parquet_path(name) -> Path` | parquet 物理 path 引き (Home cold start の `parquet_metadata` 経由 COUNT で使う) |

### lib/chip_atlas.py

ChIP-Atlas との接続点を 1 module に集約する純関数 helper。samples.parquet に `in_chip_atlas` / `chip_atlas_genome` 列を持たない代わりに、`source_system` から派生情報を取り出す。

| 関数 / 定数 | 用途 |
|---|---|
| `SOURCE_SYSTEM_TO_GENOME: dict[str, str \| None]` | 3 行 dict: `chip-atlas-hg38 → 'hg38'` / `chip-atlas-mm10 → 'mm10'` / `rnaseq-human → None`。新系統追加時はここを編集 |
| `is_chip_atlas_source(source_system) -> bool` | `source_system.startswith('chip-atlas-')` (旧 `samples.in_chip_atlas` の derived) |
| `bigwig_url(source_system, srx) -> str \| None` | `https://chip-atlas.dbcls.jp/data/{genome}/eachData/bw/{srx}.bw` を組み立て。genome が `None` の系統は `None` を返す |
| `peak_bed_url(source_system, srx) -> str \| None` | `https://chip-atlas.dbcls.jp/data/{genome}/eachData/bed05/{srx}.05.bed` を組み立て。genome が `None` の系統は `None` を返す |
...
### Fast path (`agg_*.parquet` 経由)

`build-aggregates` ETL が生成する `agg_samples_by_dims` / `agg_field_term_dims` / `agg_field_status_dims` の 3 parquet を直接スキャンする小サイズ helper 群。UI cold start で 13.3M facts や 1.65M samples を触らずに済ませる。

- 切替判定: `lib/aggregation.py:has_dashboard_aggregates(con) -> bool` が `True` のとき UI 側で `_fast` 系を呼ぶ。3 view のいずれかが欠けると `False` を返し、UI は live 関数 (`samples_by_year_source` 等) に自動 fallback (fixture / 部分 dataset での開発を壊さないため)
- filter clause: `_filter_clauses` (live, samples 別名 `s.`) と `_agg_filter_clauses` (agg, agg view 列名直接) は **invariant として同じ結果集合を返す**。NULL 塗潰しの非対称 (`samples.organism_normalized` は NULL のまま、agg 側は `'(unknown)'` sentinel) は `_filter_clauses` 側で「`'(unknown)'` が IN リストに含まれるとき `OR <col> IS NULL` を OR で展開する」ことで吸収する。`tests/pbt/lib/test_agg_parity.py` で 6 軸 × 8 fast/live pair の sum + 行集合一致を property test
- 適用範囲: Home dashboard (F1〜F5)、Curation D1/D2、Gap Discovery の top_terms (heatmap 軸選定)、Gapminder の bubble dataset。D3 (top_unmapped_values) / D5 (raw_value_term_flow) は agg では表現できないので常に live (上位カーディナリティが高すぎる)
- 関数命名: live 名に `_fast` suffix を付ける。両方 export し、UI 側で必ず `has_dashboard_aggregates(con)` 判定の if/else で切替える
...
| 関数 | 用途 |
|---|---|
| `SampleFilters` (`@dataclass(frozen=True)`) | sidebar filter snapshot。`organism_normalized` / `submission_year_min` / `submission_year_max` / `source_system` / `sequence_type` を tuple で持つ hashable dataclass。`@st.cache_data` の cache key として直接渡せる (mutable list 化 / 詰め替えを禁止) |
| `_filter_clauses(filters) -> (sql, params)` | live path。`samples s` を JOIN している前提の `s.<col> ...` 条件を組み立てる。`'(unknown)'` sentinel を IN リストに含めると `s.<col> IS NULL` も OR される |
| `_agg_filter_clauses(filters) -> (sql, params)` | agg path。agg view の dim 列を直接参照する `<col> ...`。`'(unknown)'` sentinel は agg 側でリテラル一致するため OR 展開不要 |
| `has_dashboard_aggregates(con) -> bool` | fast/live 切替判定 |
| `gap_heatmap_pivot(x_axis, y_axis, filters, overlay_axis, ...) -> pd.DataFrame` | 画面 1 (Gap Discovery)。`overlay_axis` で `secondary_count` の SQL predicate を切替 |
...
| `term_hierarchy_breakdown(con, field, filters, root_term, max_depth, roll_up_depth, by_year) -> pd.DataFrame[submission_year?,term_id,parent_term_id,label,depth,sample_count,secondary_count]` | Gap Discovery B3 / Gapminder A11 |
| `field_to_field_flow(con, x_field, y_field, filters, top_n_x, top_n_y, x_roll_up_depth, y_roll_up_depth) -> pd.DataFrame[x_term_id,x_label,y_term_id,y_label,sample_count,secondary_count]` | Gap Discovery B4 |
...
| `mixed_bs_srx_composition(con, filters, top_n=20) -> pd.DataFrame[pattern,pattern_label,n_bs,n_srx]` | Curation D7 |

#### Fast path 関数 (`agg_*.parquet` 経由)

`_fast` suffix を持つ helper 群。`has_dashboard_aggregates(con)` が `True` のときに使う。返り値の列名・型は live 版と揃える (UI 側で分岐後の処理を共通化する)。

| 関数 | 対応する live 関数 | 用途 |
|---|---|---|
| `summary_counts_fast(con) -> dict[str,int]` | (live 無し、`COUNT(*)` の dict) | Home 上部 metric (samples/chip_atlas/runs/facts/terms)。runs/facts/ontology は parquet metadata 経由で取得 |
| `samples_by_year_source_fast` | `samples_by_year_source` | Home F1 |
| `samples_by_organism_fast` | `samples_by_organism` | Home F2 左 |
| `samples_by_source_fast` | `samples_by_source` | Home F2 中央 |
| `samples_by_sequence_type_fast` | `samples_by_sequence_type` | Home F2 右 |
| `field_facts_status_fast` | `field_facts_status` | Home F3, F4 |
| `top_terms_overall_fast` | `top_terms_overall` | Home F5 |
| `mapping_status_matrix_fast` | `mapping_status_matrix` | Curation D1 |
| `mapping_status_over_time_fast` | `mapping_status_over_time` | Curation D2 |
| `top_terms_fast` | `top_terms` (roll-up=None のみ) | Gap Discovery 軸選定 / Gapminder bubble の TopN |
| `bubble_dataset_fast` | `bubble_dataset` (roll-up=None のみ) | Gapminder Bubble / Trajectory / Heatmap |
